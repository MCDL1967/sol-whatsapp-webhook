# Demo Data to Schema Mapping v0.3
...
## v0.3 Accepted Operational Mapping

Operational records should map to the hybrid case model:

```text
guest conversation -> guest_threads + messages
submitted/actionable request -> cases
reservation facts -> reservation_details
complaint facts -> complaint_details
incident facts -> incident_details
generic service facts -> service_request_details
business/audit actions -> operational_events
```

Retired proposal mappings:
...
Reservation drafts remain transient until submitted/finalized.
