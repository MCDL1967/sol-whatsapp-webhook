...
- **`tenant_tool` rework pass — flagged 2026-08-04, not yet scoped.** A follow-up pass on `tenant_tool` after the incident/complaint write path (and likely touching the same surface as the webhook rewire), covering: (1) how tenant-typed text actually reaches guests through `webhook.js` — spacing, emoji, and other formatting details in what a tenant types vs. what gets sent, not an issue today since nothing tenant-typed reaches a guest live yet, but real once the rewire lands; (2) Menu Tree subsections beyond this pass's deliberately structure-only scope (labels and hierarchy only — see section 5's 2026-08-03 resolution); (3) live previews of a section's guest-facing effect (e.g. a menu branch rendered as a WhatsApp list, a response template with variables filled in) directly in the tool — no preview rendering exists anywhere in `tenant_tool` today.
...
- **Queued for next session: per-branch intro/fallback message, replacing the hardcoded `GENERIC_FALLBACK_TEMPLATE`.** Directly caused by the preview above surfacing how many options (especially any new one a tenant adds) have no configured response at all. This is the same gap already named during the webhook-rewrite planning ("Reprompt templates" decision) — at the time, deliberately deferred in favor of one hardcoded property-wide fallback string in `fast_path_responder.js`, with a real `menu_branches` reprompt/intro column explicitly left as later, additive work. That's now the next concrete step: a per-branch (level) intro message, stored in Supabase rather than hardcoded in JS, that a `tenant_tool`-created option without its own `template_key` falls back to instead of the generic string — giving every level a real, tenant-configurable default rather than a dead end. Not started; scope this properly (new column vs. reusing something existing, tenant_tool UI to set it) before touching code, same as every other multi-decision piece this project.
