...
## v0.3 Accepted Boundaries

- Runtime package recommendation is accepted.
- The service consumes the published package, not raw normalized tables during guest turns.
- The service does not write `cases`, `messages`, or `operational_events` directly unless a later implementation explicitly assigns that responsibility.
- Voiceflow remains the current conversational runtime boundary.
- Direct LLM orchestration is deferred.
- Voiceflow session/init variables should be derived from approved DB/config variables.
- Technical runtime errors remain in logs unless they directly affect a case and need manager visibility.
