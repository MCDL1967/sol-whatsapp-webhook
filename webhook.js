...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Decide BEFORE any updates whether this turn must force flow entry
    const forceLaunch = session.state === "idle" || isExplicitReset(userText);

    // ---- REQUEST CONTROL ----
    if (isExplicitReset(userText)) {
      updateSession(userID, {
        active_request: null,
        state: "idle"
      });

      console.log(`[REQUEST RESET] user=${userID} reason=explicit_reset_phrase`);
    } else {
