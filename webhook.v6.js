...
function getSafetyFallbackMessage(text) {
  ...
  const spanishSignals = /[áéíóúñ¿¡]/i.test(text || "") ||
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (replies.length === 0) {
      ...
      console.log(
        `[VOICEFLOW] No text reply for user=${userID} session_id=${sessions[userID].session_id} action=${vfAction.type} -> sending middleware fallback`
      );
